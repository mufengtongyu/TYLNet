...
def get_norm_layer(norm_type):
    ...
    elif norm_type == 'none': norm_layer = None
...
def get_scheduler(optimizer, config):
    ...
    elif config['lr_policy'] == 'step': scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=config['scheduler_epoch'], gamma=float(config['gamma']))
    ...
    elif config['lr_policy'] == 'cosine': scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(config['niter']), eta_min=0)
    else: return NotImplementedError('learning rate policy [%s] is not implemented', config['lr_policy'])
    return scheduler
...
def init_weights(net, init_type, init_gain):
    ...
    def init_func(m):
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            ...
            else: raise NotImplementedError('initialization method [%s] is not implemented' % init_type)
